...
add_action( 'genesis_meta', 'wpdt_home_page_setup');
/**
...
function wpdt_home_page_setup() {
    ...
    if ( $home_sidebars['home_welcome'] ) {
        add_action( 'genesis_after_header', 'wpdt_add_home_welcome' );
    }

    // Add call to action area if "Call to Action" widget area is active.
    if ( $home_sidebars['call_to_action'] ) {
        add_action( 'genesis_after_header', 'wpdt_add_call_to_action' );
    }
 ...
 * @since 1.0.0
 */
function wpdt_add_home_welcome() {
 ...
 * @since 1.0.0
 */
function wpdt_add_call_to_action() {
